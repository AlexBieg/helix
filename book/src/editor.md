...
| `picker-animation` | Entrance animation played when a picker opens. Can be `none`, `unfold` (grow down), `unfold-horizontal` (grow from center), `unfold-both` (zoom from center), or `cascade` (rows fill in) | `"unfold-both"` |
