...
| `blame` | Whether to show git blame information below the cursor line. Toggle with `toggle_blame` (`Space m`). Requires the `git` feature. | `false` |
...
| `rounded-corners` | Set to `true` to draw rounded border corners | `true` |
...
| `picker-animation` | Entrance animation played when a picker opens. Can be `none`,
`unfold` (grow down), `unfold-horizontal` (grow from center), `unfold-both`
(zoom from center), or `cascade` (rows fill in) | `"none"` |
